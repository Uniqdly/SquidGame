...
public class CutChecker : MonoBehaviour
{
    ...
    public float tolerance = 0.02f;          // допустима€ дистанци€ до точки
    public float maxNoContactTime = 0.6f;    // если игла не касаетс€ штампа дольше этого - consider miss
    public float minTimeBetweenSamples = 0.02f;

    [Header("Miss settings")]
    public int maxAllowedMisses = 3;         // на 3ем Ч исчезание и смерть
    public float missCooldown = 0.6f;        // минимальное врем€ между засчитанными промахами
    ...
    {
        Debug.LogError("CutChecker: FINAL FAIL (max misses) Ч breaking candy and killing player.");
        failed = true;
        active = false;
