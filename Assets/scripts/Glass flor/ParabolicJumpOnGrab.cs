...
public class ParabolicJumpOnGrab : MonoBehaviour
{
    ...
    [Tooltip("¬ертикальный оффсет от поверхности при посадке (м)")]
    public float landingYOffset = 0.05f;
    [Tooltip("¬рем€ в секундах, которое игрок будет замирать на центре платформы после приземлени€")]
    ...
    void Update()
    {
        ...
        if (jumpPressed && !prevJump)
        {
            if (highlighted != null)
            {
                // провер€ем cooldown между прыжками
                ...
                {
                    prevJump = jumpPressed; // сохран€ем текущее состо€ние
    ...
    {
        // 1) Action-based (preferred) Ч supports multiple bindings (keyboard + controller)
        if (jumpAction != null && jumpAction.action != null)
        {
    ...
    IEnumerator ParabolicJumpTo(Transform target)
    {
        ...
        // --- вычисл€ем корректную целевую позицию (центр поверхности target) ---
        Vector3 center = target.position;
        bool foundBounds = false;

        // 1) попробуем Renderer.bounds (обычно лучше дл€ визуального центра)
        ...
        {
            // 2) если Renderer нет Ч попробуем Collider.bounds (например у плитки есть коллайдер)
            ...
            {
                // 3) если и этого нет Ч попытаемс€ собрать все renderers в родител€х/дет€х и усреднить центры
        ...
        // endPos: по горизонтали Ч центр поверхности, по вертикали Ч оставл€ем высоту игрока (не занижаем голову)
        ...
        // финальна€ позици€ Ч немного выше поверхности, чтобы избежать пересечени€
        ...
        // сразу ставим позицию Ч игрок оказываетс€ в центре платформы
        playerRoot.position = finalPos;

        // --- ѕј”«ј: игрок замирает на центре платформы на landingPause секунд ---
        if (landingPause > 0f)
        {
            yield return new WaitForSeconds(landingPause);
        }

        // --- затем ждЄм один физический шаг и стабилизируем Rigidbody, чтобы избежать подпрыгов ---
        if (hadRb)
        {
        ...
        else
        {
            ...
            {
                Debug.Log("[PJ] Landed on fragile " + gp.name + " Ч calling Break()");
                gp.Break();
            }
        }
