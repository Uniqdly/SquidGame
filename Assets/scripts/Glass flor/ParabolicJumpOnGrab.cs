...
public class ParabolicJumpOnGrab : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (jumpPressed && !prevJump)
        {
            if (highlighted != null)
            {
                if (requireGrounded && !IsGrounded()) { prevJump = jumpPressed; return; }
    ...
    IEnumerator ParabolicJumpTo(Transform target)
    {
        ...
        float landingYOffset = 0.05f;
        // если у нас нашли bounds — используем верхнюю границу bounds.max.y как поверхность (точнее)
        if (foundBounds)
        {
            float surfaceY = float.MinValue;
            // если был renderer — surface is renderer.bounds.max.y, иначе collider.bounds.max.y
            ...
            {
                Vector3 finalPos = new Vector3(center.x, surfaceY + landingYOffset, center.z);
                playerRoot.position = finalPos;
            }
            else
            {
                playerRoot.position = new Vector3(endPos.x, endPos.y + landingYOffset, endPos.z);
            }
        }
        else
        {
